...
#theorem(title: ctext("可控性（controbility）"))[
...
- 若渐近稳定，则$a < 0, b=0$
- 若$b≠0$则必有共轭，且$b i$必引入振动

#tip[
  简单说，$λ$决定收敛速度，若$∃ λ > 0$，则系统不稳定。
]

#theorem(title: ctext("Lyapunov 稳定性"))[
  原点（起始点）稳定，当且仅当
  $ ∀t_0, ∀ɛ > 0, ∃δ(t_0, ɛ): |x(t_0)| < δ(t_0, ɛ) ⇒ ∀t ≥ t_0 med |x(t)| < ɛ $
]

#theorem(title: ctext("渐近稳定性"))[
...
$ G(s) = frac(N(s), D(s)) = frac(sum_(i=1)^m (s - z_i), sum_(j=1)^n (s - p_j)) $
...
#theorem(title: ctext("Nyquist-Shannon 抽样定理"))[
  抽样有效性（重建原信号）需满足
  抽样频率 ≥ 被采信号最高频率 × 2
]
...
实际使用中，$ω_R$往往为低频常数，而来自传感器的$ω_N$往往为高频常数。此时，将$T(s)$设计为一个低通滤波器，即可解决噪声问题。对于扰动，需要做如下考虑，即由

$ T(s) = frac(C(s) G(s), 1 + C(s) G(s)) $

且假设$C(s) G(s) = a + b j$，可知
