#import "lib/sci-book.typ": *
#show: doc => conf(
  title: "最优控制",
  header: "现代控制理论",
  author: ("ivaquero"),
  header-cap: "github/ivaquero",
  outline-on: true,
  doc,
)
#show: thmrules

= 性能指标
<性能指标>

== 位置控制
<位置控制>

对独轮车模型

#figure(
  image("./images/vehicle/unicycle.drawio.png", width: 40%),
  caption:[独轮车],
  supplement: [图]
)

令

- 位置
  - $x_1(t) = p_x(t)$
  - $x_2(t) = p_y(t)$
- 速度
  - $x_3(t) = v(t)$
- 角速度
  - $x_4(t) = θ(t)$

于是有

$ 𝒙(t) = mat(delim: "[", p_x(t); p_y(t); v(t); θ(t)) $

设计输入（控制量）

$ 𝒖(t) = mat(delim: "[", u_1(t); u_2(t)) = mat(delim: "[", α_1(t); ω_2(t)) $

可得

$ frac(dif 𝒙(t), dif t) =
mat(delim: "[", v(t) cos θ(t); v(t) sin θ(t); 0; 0) +
mat(delim: "[", 1; 0; α(t); ω(t)) = f(𝒙(t)), 𝒖(t) $

== 离散化
<离散化>

对时间$t$离散化，有

$ 𝒙[k + 1] = f_d [𝒙[k], 𝒖[k]] $

设目标为

$ 𝒙_d = mat(delim: "[", p_(x d)(t); p_(y d)(t); 0; 0) $

设计代价函数

$ J &= ∑_(i=1)^4 (x_i [N] - x_(i d))^2\
 &= (𝒙[N] - 𝒙_d)^(⊤)(𝒙[N] - 𝒙_d) $

寻找合适的控制策略，使$J$最小

$
𝒖^* =[𝒖[0], 𝒖[1], 𝒖[2], …, 𝒖[N-1]] \
𝒖^* = "arg" min J $

考虑实际因素，每个状态变量的重要性不同，于是有

$ J
&= 1/2 (𝒙[N] - 𝒙_d [N])^(⊤) 𝑺 (𝒙[N] - 𝒙_d [N])\
&= 1/2 (𝒙[N] - 𝒙_d [N])^(⊤)
  mat(delim: "[",
  S_1, med, med, med;
  med, S_2, med, med;
  med, , S_3, med;
  med, med, med, S_4)(𝒙[N] - 𝒙_d [N])\
&= 1/2 ∑_(i=1)^4 S_i (x_i [N] - x_(i d) [N])^2\
&= 1/2 norm(𝒙[N] - 𝒙_d [N])_𝑺^2 $

其中，$𝑺_(n × n)$为半正定对称阵，$1/2 norm(𝒙[N] - 𝒙_d [N])_𝑺^2$称末端代价。

== 实际约束
<实际约束>

考虑物理约束（硬约束）

- 速度
- 控制量

同时，考虑能耗，有

$ J = 1/2 lr(norm(𝒙[N] - 𝒙_d [N]))_𝑺^2 + 1/2 ∑_(k=1)^(N - 1) norm(𝒖[k])_𝑹^2 $

其中，$𝑹 = mat(delim: "[", r_1, med; med, r_2)$为正定对称阵。由于其通过算法对$J$进行约束，故属于软约束

- 不考虑能耗，则$𝑺 ≫ 𝑹$
- 关注节能，则$𝑹 ≫ 𝑺$

#tip[
  $𝑹_(p × p)$正定时，$J$方有最小值。
]

在硬、软约束的基础上，对轨迹进行约束，有

$ J = 1/2 norm(𝒙[N] - 𝒙_d [N])_𝑺^2 + 1/2 ∑_(k=1)^(N - 1)(norm(𝒙[k]) - 𝒙_d [k])_𝑸^2 + norm(𝒖[k])_𝑹^2 $

其中，$𝑸_(n × n)$为半正定对角阵，$1/2 norm(𝒙[k] - 𝒙_d [k])_𝑸^2$称运行代价。

又轨迹中间点，$p_x[k], p_y[k] in p_x^*[k], p_y^*[k]$，则

$ 𝒙_1[k], 𝒙_2[k] in X^* $

称为容许轨迹（admissible trajectory）。而控制量约束

$ 𝒖^* ∈ Ω $

称为容许控制域（set of admissible control）。

= 动态规划
<动态规划>

动态规划由 Richard Bellman 于 1966 年在《Science》上发表。

== 高度控制
<高度控制>

对无人机模型，现需要求解，从地面升高至 10m 的最短时间

#figure(
  image("./images/vehicle/drone.drawio.png", width: 40%),
  caption:[无人机],
  supplement: [图]
)

初始和最终条件为

- $h(0) = 0$
- $dot(h)(0) = 0$
- $h_f = 10$
- $dot(h)_f = 0$

物理约束为

- $a(t) in[-3, 2]$
- $v(t) in[0, 3]$

关系式为

$ m dot.double(h)(t) = f(t) - m g $

控制量为

$ u = 1/m (f(t)) - m g = a $

其中，$f(t) = F(v_("motor")$

于是有

- $macron(v) = 1/2 (v (k)) + v (k + 1))$
- $t = dif x/1/2 (v_k + v_(k + 1))$
- $a = (v_k + v_(k + 1))/t$

== 策略
<策略>

- 以终为始
- 空间换时间
