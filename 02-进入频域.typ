...
卷积积分似乎相当复杂，对于任意输入进行积分会非常繁琐。不仅要进行积分困难，卷积也不能让我们轻松地将多个系统组合成一个大型系统。那么，什么会帮助我们呢？是传递函数。我们可以使用传递函数进行卷积，这里我们使用乘法而不是积分来进行。为了继续我们的旅程，我们需要离开时域，并进入频域。

== 弹簧阻尼系统

乍一看，进入频域似乎是一个不便的步骤，但它绝对值得付出努力。想象一下一个静止在弹簧上的一质点。若我们施加一个脉冲力作用于该质点，它将像弹簧玩具一样上下弹跳。若系统没有阻尼，或者没有能量损失，那么质点将永远上下弹跳。这总是很难在时域中绘制，更重要的是，在只看到系统时域响应的情况下，有时很难观察到有意义的行为。
...
通过假设解的形式为 $x(t) = A cos(ω t +φ)$，然后求解三个未知系数$A$、$ω$ 和 $φ$。 由于有三个未知数，我们需要三个方程来求解它们。对于第一个方程，我们可以计算$x(t)$的二阶导数，然后将$x(t)$插入运动方程中。后两个方程来自两个已知的初始条件。我们知道初始位置为零，即$x(0) = 0$。 并且我们知道以前的瞬时力产生的结果是，瞬时速度等于物体质量的倒数，即$dot(x)(0) = −1 \/ m$。

由于我们已经将输入力计为初始速度，因此我们在第一个方程中将力设为零。 在一些代数运算后，我们得到弹性的频率 $ω = sqrt(k \/ m)$，初始的起跳点 $φ = π \/ 2$，以及弹性的幅度 $A = 1 \/ sqrt(k m)$。
