...
在线性系统中，如果系统的动态特性不随时间推移而改变，则称为**线性时不变（Linear Time-Invariant, LTI）系统**，也称为线性自治系统。其关键特征是描述系统的微分方程系数为常数。其一阶状态空间方程通常表示为

$ dot(x) = A x + B u $

其中矩阵$A$和$B$均为常数矩阵。LTI 系统最重要的性质是时不变性，即输入信号的时间延迟只会引起输出信号相同的延迟，而响应的波形不变。数学上可表示为：若$O\{f(t)\} = x(t)$，则$O\{f(t - τ)\} = x(t - τ)$。

与之相对，如果一个线性系统的动态特性会随时间发生变化，则称为**线性时变（Linear Time-Varying, LTV）系统**，也称为线性非自治系统。其微分方程的系数是时间的函数，其一阶状态空间方程通常表示为

$ dot(x) = A(t) x + B(t) u $

其中矩阵$A(t)$和$B(t)$的元素均随时间变化。
