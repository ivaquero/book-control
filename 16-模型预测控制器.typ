...
$ min J = underbrace(𝒛^(⊤) 𝑸 𝒛, "二次型") + underbrace(𝑪^(⊤) 𝒛, "线性") $ <cost>
...
1. 估计/测量$k$时刻系统状态；
2. 基于代价函数进行最优化控制。离散化@eqt:costc，得

$ J = ∑_k^(N-1) (𝒆^(⊤)_k 𝑸 𝒆_k + 𝒖^(⊤)_k 𝑹 𝒖_k) + 𝒆^(⊤)_N 𝑭 𝒆_N $ <costd>

3. 以$𝒖_k^*$作为输入，重复第2步，进行继续循环；

== 代价函数推导

MPC 问题的一个难点就是最小化代价函数的计算。目前的思路是将@eqt:costd 转化为二次型，这里令
...
为将@eqt:costd2 化为@eqt:cost 形式，即只有一个变量的形式，做如下代换
