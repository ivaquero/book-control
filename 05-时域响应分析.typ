...
$ dv(e^(𝑨 t), t) = sum_(i=1) frac(1, (i - 1))! 𝑨^i t^(i - 1) = 𝑨 e^(𝑨 t) $
...
其中，$ϕ = arctan c_1\/c_2$。此时，系统振幅在震动中衰减，且震动周期为$2pi\/ω_d$。

#tip[
  欠阻尼是日常生活中最常见的状态
]

- 当$ζ = 0$，此时系统处于无阻尼状态，该极点为中心点。此时有解，图像为正弦函数，周期为$2pi\/ω_n$
...
- 正弦函数$sin(ω_dd(t) + ϕ)$的频率为$w_d$（周期为$2pi / w_d$）
