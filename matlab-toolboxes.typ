...
名称空间（也称为软件包）提供了一种组织类和功能的方法，并降低了具有相同名称的两个功能的风险。请参阅：#link("https://mathworks.com/help/matlab/matlab_oop/scoping-classes-with-packages.html")

=== MATLAB Apps

MATLAB 应用程序是交互式图形应用程序，允许用户在工具箱中执行特定的工作流程。我们将 MATLAB 应用程序包装到一个文件（.mlapp）中，以更轻松地分发。在工具箱文件夹的顶层创建“应用程序”文件夹。打包工具箱时，请确保将我们的应用程序包含在工具箱包装对话框的应用程序部分中。这样，用户可以在安装后轻松访问和运行我们的应用程序。请参阅：#link("https://mathworks.com/help/matlab/gui-development.html")[GUI-Development]。

=== 实时任务

实时任务是简单的点击接口，可以在 R2022a 开始的实时脚本中使用。它们为用户提供了一种交互式和直观的方法，可以与我们的工具箱进行交互。将实时任务类放在工具箱文件夹中的“内部”文件夹中，因为用户不直接调用此功能。作为创建的一部分，我们将创建一个 `liveTasks.json` 文件，该文件必须在 `resources` 文件夹中。请参阅：#link("https://mathworks.com/help/matlab/develop-live-editor-tasks.html")[Develop-Live-Editor-Tasks]。
...
我们可以为工具箱添加版本编号和其他信息提供图标。用户可以通过 #link("https://mathworks.com/help/matlab/matlab_env/get-add-ons.html")[Add-on Manager] 轻松发现，安装，更新和卸载工具箱。请参阅：#link("https://mathworks.com/help/matlab/matlab_prog/create-and-share-custom-matlab-toolboxes.html")[Create-And-Share-Custom-Matlab-Toolboxes]。
...
#figure(
  image("images/toolbox.png", width: 45%),
  caption: none,
)
...
- 创建一个 #link("https://docs.github.com/en/repositories/releasing-projects-on-github/managing-releases-in-a-repository")[GitHub 仓库]，并链接到 #link("https://mathworks.com/matlabcentral/fileexchange/")[MATLAB File Exchange]。工具箱将在 `Add-on Explorer` 中出现，并将安装最新的发布版本。请参阅 #link("https://mathworks.com/matlabcentral/discussions/highlights/132204-github-releases-in-file-exchange")[Github-Releases-In-File-Exchange]。
...
测试检查工具箱的质量并帮助我们建立信心高质量的发布。#link("https://mathworks.com/help/matlab/matlab-unit-test-framework.html")[MATLAB Testing Framework] 提供了用于测试代码的支持。对于熟悉 MATLAB 的用户，应该很熟悉#link("https://mathworks.com/help/matlab/function-based-unit-tests.html")[Function-Based Unit Tests]。
...
#link("https://mathworks.com/help/matlab/projects.html")[工程文件] 是确保创作团队保持一致环境的好方法。它管理复杂项目中的依赖性，使路径正确，并与源控制系统集成。
...
与打包有关的脚本应放置在根文件夹下方的`buildUtilities`文件夹中。 考虑使用 R2022b 中引入的 #link("https://mathworks.com/help/matlab/matlab_prog/overview-of-matlab-build-tool.html")[buildtool]。与 `buildtool` 关联的任务功能在 `buildfile.m` 中。
...
#link("https://mathworks.com/products/matlab-online/git.html")[MATLAB Online] 给出了在线运行
