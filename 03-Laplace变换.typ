...
@inv 表示$x(t)$由无数个无穷小的小波相加而成，而 $X(s)$则表示$s$平面上每个点需要多少个小波。该加权因子由变换给出（@laplace）。分解出的每个小波都由复数$X(s)$ 加权。​​然后，它们通过传递函数$G$，该函数现在不再是$G(j ω)$（仅针对正弦波定义），而是针对整个复平面定义的$G(s)$。$X(s)G(s)$的结果表示输出中包含的$s$平面上每个点的$e^(s t)$量。$X(s)G(s)$上使用@inv，可让我们回到时间域并得到输出。
