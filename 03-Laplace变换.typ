...
所有线性系统都遵循叠加（superposition）。若输入$x(t)$可以分解为正弦波的总和，使用傅里叶变换来找到它们的振幅（对于离散的、时间有限的系统，产生的频率是 ω0 的倍数）。每个都通过$G(j ω)$。$G(j ω)$具有增益和相移，这取决于频率。然后可以将产生的正弦波$G(j n ω_0)e^(j n ω_0 t)$全部加起来。
