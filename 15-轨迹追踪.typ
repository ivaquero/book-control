...
$
  mat(delim: "[", λ, 1; g / L - k_1, λ - k_2) & = λ^2 - k_2 λ - g / L + k_1 \
                                              & = λ^2 + 2 λ + 1
$
...
$
  J &= 1 / 2 𝒆^⊤_([N]) 𝑺 𝒆_([N]) + 1 / 2 sum_(k=0)^(N-1) (𝒆^⊤_([k]) 𝑸 𝒆_([k]) + 𝒖^⊤_([k]) 𝑹 𝒖_([k]))\
  &= 1 / 2 (𝑪_a 𝒙_(a[k]))^⊤_([N]) 𝑺 (𝑪_a 𝒙_(a[k])) + 1 / 2 sum_(k=0)^(N-1) ( (𝑪_a 𝒙_(a[k]))^⊤_([k]) 𝑸 (𝑪_a 𝒙_(a[k]))_([k]) + 𝒖^⊤_([k]) 𝑹 𝒖_([k]) )\
  &= 1 / 2 𝒙_(a[k])^⊤ 𝑺_a 𝒙_(a[k]) + 1 / 2 sum_(k=0)^(N-1) (𝒙_(a[k])^⊤ 𝑸_a 𝒙_(a[k]) + 𝒖^⊤_([k]) 𝑹 𝒖_([k]))
$ <cost-spring>
