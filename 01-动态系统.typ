...
- 黑盒方法：你将包装框中的内容对其进行各种已知输入，测量结果输出，然后根据两者之间的关系来推断框中的内容。
- 白盒方法：你在了解系统的组件后，直接编写动力学的数学方程式。

通常，我们会同时使用这两种方法。

== 仿真测试

仿真问题是在考虑到已知的输入集和系统的数学模型的情况下，预测输出如何变化。你可能会在此阶段花费大量设计时间。这里的诀窍是弄清一组有意义的输入及其范围，以便对系统的行为有一个完整的了解。这里你需要提出以下问题
