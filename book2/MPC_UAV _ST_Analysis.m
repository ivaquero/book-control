...
%% 作者：王天威，黄军魁
%% 清华大学出版社
%% 程序名称：MPC_UAV_ST_Analysis.m
%% 程序功能：无人机高度速度模型预测控制 （采样时间、预测区间分析，5.6.2节案例）
%% 所用模块：
%%        [F2]稳态非零控制矩阵转化模块
%%        [F4]性能指标矩阵转换模块
%%        [F6]约束条件矩阵转换模块
...
% 定义程序运行时间向量，用于存储计算时间，考量实时控制可行性
elapsed_time_history = zeros(1,k_steps);
...
%%%%%%%%%%%%%%%%%结果%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(1, 'position',[150 50 500 800]);
% 状态变量结果图
subplot  (3, 1, 1);
hold;
% 无人机高度
...
subplot (3, 1, 3);
hold;
plot (0:length(elapsed_time_history)-1,elapsed_time_history(1,:),"linewidth", 2);
grid on
xlim([0 k_steps]);
